...
from pbt import Member, BatchGenerator
...
class TestsPbtMethods(unittest.TestCase):
    ...
    def create_batch_generator(self):
        return BatchGenerator(self.x_train, self.y_train, self.x_test, self.y_test, batch_size=BATCH_SIZE)

    def test_step(self):
        """Training two members of the population with the same parameters. Losses should be equal."""
    ...
    def test_explore(self):
        """Training two members of the population. Calling 'explore' in one of them. Losses should be different."""
        member_a = Member(self.create_batch_generator())
        member_a.step()
        loss_a, accuracy_a = member_a.step()
        member_a_reg_values = member_a.regularizer.get_config()
        ...
        loss_b, accuracy_b = member_b.step()
        member_b_reg_values = member_b.regularizer.get_config()
        self.assertRaises(AssertionError, self.assertDictEqual, member_a_reg_values, member_b_reg_values)
        self.assertRaises(AssertionError, self.assertSequenceEqual, [loss_a, accuracy_a], [loss_b, accuracy_b])
